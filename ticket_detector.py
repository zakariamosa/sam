...
def ocr_confidence_for_ticket(image_bgr: np.ndarray, ticket: TicketMask, psm: int) -> tuple[float, int]:
...
def apply_ocr_confidence_check(
    ...
    for ticket in tickets:
        confidence, word_count = ocr_confidence_for_ticket(image_bgr, ticket, config.ocr_psm)
        ticket.ocr_confidence = None if confidence < 0 else confidence
        ticket.ocr_word_count = word_count

        if confidence < 0:
            if not warned_unavailable:
                print("OCR check skipped: pytesseract or tesseract executable is unavailable.", file=sys.stderr)
